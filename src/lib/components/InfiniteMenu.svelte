<script>
	...
	class InfiniteGridMenu {
		...
		#init(onInit) {
			...
			this.discVAO = makeVertexArray(
				...
				[
					[
						makeBuffer(gl, this.discGeo.data.vertices, gl.STATIC_DRAW),
						this.discLocations.aModelPosition,
						3
					],
					[makeBuffer(gl, this.discGeo.data.uvs, gl.STATIC_DRAW), this.discLocations.aModelUvs, 2]
				],
				this.discGeo.data.indices
			);
		...
		#render() {
			...
			gl.drawElementsInstanced(
				gl.TRIANGLES,
				this.discGeo.data.indices.length,
				gl.UNSIGNED_SHORT,
				0,
		...
		#onControlUpdate(deltaTime) {
			...
			if (isMovingFlag !== this.movementActive) {
				this.movementActive = isMovingFlag;
				this.onMovementChange(isMovingFlag);
			}
	...
	onMount(() => {
		const handleActiveItem = (index) => {
			activeItem = items[index % items.length];
		};
...
<div class="relative h-full w-full">
	<canvas
		bind:this={canvasRef}
		class="relative h-full w-full cursor-grab overflow-hidden outline-none active:cursor-grabbing"
	></canvas>
	...
	{#if activeItem}
		...
		<div
			class="absolute bottom-20 left-1/2 z-10 grid h-16 w-16 -translate-x-1/2 cursor-pointer place-items-center rounded-full border-4 border-white bg-[#8c2bee] text-white shadow-2xl transition-all duration-500 ease-out hover:scale-110 active:scale-95 {isMoving
				? 'translate-y-20 scale-0 opacity-0'
				: 'translate-y-0 scale-100 opacity-100'}"
			onclick={handleButtonClick}
		>
			<span class="relative top-[2px] text-3xl select-none">&#x2197;</span>
		</div>
	{/if}
</div>
