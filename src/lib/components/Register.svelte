...
<div class="pt-20 font-['Plus_Jakarta_Sans'] text-[#181115]">
				...
				<div class="flex flex-col gap-6 lg:col-span-8">
					...
					<div class="flex flex-col gap-4">
						...
						<p class="text-lg text-[#896179]">
							{#if isDeptLocked && selectedDept}
								Registering for <span class="font-bold text-[#8c2bee]">{selectedDept.fullName}</span
								> Symposium.
							{:else}
					...
					<div class="rounded-xl border border-[#f4f0f3] bg-white p-6 shadow-sm">
						<!-- Type Toggle (Hidden if Department is Locked) -->
						{#if !isDeptLocked}
                    ...
                    {registrationType === 'cultural'
						...
						<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2">
							...
							<div class="flex flex-col gap-2">
								<label for="year" class="text-sm font-bold">Year of Study</label>
								<select
									id="year"
							...
							<div class="flex flex-col gap-2 md:col-span-2">
								<label for="college" class="text-sm font-bold">College Name</label>
								<input
									id="college"
									bind:value={formData.college}
									class="h-11 w-full rounded-lg border border-[#e6dbe1] bg-transparent px-4 transition-all outline-none focus:border-transparent focus:ring-2 focus:ring-[#8c2bee]"
									placeholder="Trichy Engineering College"
									type="text"
								/>
							</div>
							<div class="flex flex-col gap-2 md:col-span-2">
								...
								<textarea
									...
									class="min-h-[80px] w-full rounded-lg border border-[#e6dbe1] bg-transparent p-3 transition-all outline-none focus:border-transparent focus:ring-2 focus:ring-[#8c2bee]"
									placeholder="Enter your full address"
								></textarea>
							</div>
							<div class="flex flex-col gap-2">
								<label for="food" class="text-sm font-bold">Food Preference</label>
								<select
									id="food"
						...
						<!-- SYMPOSIUM SELECTION -->
						{#if registrationType === 'symposium'}
							<div in:fade id="dept-selection">
								<h3 class="mb-4 flex items-center gap-2 text-lg font-bold">
									<Settings size={20} class="text-[#8c2bee]" />
									{isDeptLocked ? 'Selected Department' : 'Select Department'}
								</h3>

								<!-- Dept Grid -->
								<div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
									{#each symposiumData as dept, idx}
										<!-- If Locked, only show the selected department. If not locked, show all. -->
										{#if !isDeptLocked || selectedDeptIndex === idx}
											<button
                                                ...
                                                {isDeptLocked
													? 'pointer-events-none w-full cursor-default sm:col-span-2'
													: ''}
                                                "
												on:click={() => {
													if (!isDeptLocked) {
					...
					<div class="p-6 text-left">
						<div class="mb-6 grid grid-cols-2 gap-6">
							...
							<div>
								...
								<p class="truncate text-base font-bold">{formData.college}</p>
							</div>
							<div>
								<p class="mb-1 text-xs font-bold tracking-widest text-[#896179] uppercase">Food</p>
								<p class="text-base font-bold">{formData.food}</p>
							</div>
						</div>
